...
public class MessageController {
    ...
    private static final String ROOM1408 = "1408";
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        if (roomId.equals(ROOM1408)) {
            logger.info("cant send messages from room 1408");
            return;
        }
    ...
    private void templateSend(Message message, String roomId) {
        String msgEscaped = HtmlUtils.htmlEscape(message.messageStr());
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId),
                new Message(msgEscaped));
